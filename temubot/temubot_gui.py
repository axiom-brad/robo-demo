...
class SerialApp:
    ...
    def __init__(self, root):
        ...
        # Increase window size to make all elements and the serial monitor visible
        self.root.geometry("800x800")
        self.root.configure(bg="#2c3e50")

        # Serial port configuration
        self.serial_port = '/dev/ttyACM0'  # Adjust this to your serial port
        self.baud_rate = 9600
        self.ser = None
        self.stop_event = threading.Event()  # Event to stop the serial reader thread
    ...
    def setup_gui(self):
        ...
        title_label = tk.Label(main_frame, text="Motor Angle Controller", font=("Helvetica", 24, "bold"), fg="#ecf0f1", bg="#34495e")
        ...
        self.sliders = []
        for i in range(1, 4):
            label = tk.Label(slider_frame, text=f"Motor {i} Angle", font=("Helvetica", 14), fg="#ecf0f1", bg="#34495e")
            label.pack(pady=2)
            # Create a slider with a range of -720 to 720 degrees
            slider = tk.Scale(slider_frame, from_=-720, to=720, orient=tk.HORIZONTAL, length=600,
                              bg="#7f8c8d", fg="#2c3e50", troughcolor="#bdc3c7", font=("Helvetica", 12),
                              highlightthickness=0, borderwidth=0, resolution=0.01)
        ...
        # Ping button
        ping_button = tk.Button(top_button_frame, text="Ping", font=("Helvetica", 14, "bold"),
                                bg="#3498db", fg="#ecf0f1", activebackground="#2980b9", width=12,
                                command=lambda: self.send_serial_command("ping\n"))
        ping_button.pack(side=tk.LEFT, padx=5)

        # Reset button
        reset_button = tk.Button(top_button_frame, text="Reset", font=("Helvetica", 14, "bold"),
                                 bg="#f1c40f", fg="#2c3e50", activebackground="#f39c12", width=12,
                                 command=self.reset_sliders)
        reset_button.pack(side=tk.LEFT, padx=5)

        # Get Motor Angle buttons
        get_angle_frame = tk.Frame(button_frame, bg="#34495e")
        get_angle_frame.pack(pady=5, anchor=tk.W)

        get_angle1_button = tk.Button(get_angle_frame, text="Get Angle 1", font=("Helvetica", 14, "bold"),
        ...
        get_angle2_button = tk.Button(get_angle_frame, text="Get Angle 2", font=("Helvetica", 14, "bold"),
                                       bg="#2ecc71", fg="#ecf0f1", activebackground="#27ae60",
                                       command=lambda: self.send_serial_command("get angle 2\n"))
        get_angle2_button.pack(side=tk.LEFT, padx=5, pady=5)

        get_angle3_button = tk.Button(get_angle_frame, text="Get Angle 3", font=("Helvetica", 14, "bold"),
                                       bg="#2ecc71", fg="#ecf0f1", activebackground="#27ae60",
        ...
        move_motor1_button = tk.Button(move_motor_frame, text="Move Motor 1", font=("Helvetica", 14, "bold"),
        ...
        move_motor2_button = tk.Button(move_motor_frame, text="Move Motor 2", font=("Helvetica", 14, "bold"),
                                        bg="#e74c3c", fg="#ecf0f1", activebackground="#c0392b",
                                        command=lambda: self.move_motor(2))
        move_motor2_button.pack(side=tk.LEFT, padx=5, pady=5)

        move_motor3_button = tk.Button(move_motor_frame, text="Move Motor 3", font=("Helvetica", 14, "bold"),
                                        bg="#e74c3c", fg="#ecf0f1", activebackground="#c0392b",
        ...
        self.action_buttons = [ping_button, reset_button, get_angle1_button, get_angle2_button, get_angle3_button,
                               move_motor1_button, move_motor2_button, move_motor3_button]

        # Serial Monitor Display
        display_frame = tk.Frame(main_frame, bg="#34495e")
        display_frame.pack(expand=True, fill="both")

        display_label = tk.Label(display_frame, text="Serial Monitor", font=("Helvetica", 14, "bold"), fg="#ecf0f1", bg="#34495e")
        display_label.pack(pady=(0, 5))

        self.display_text = tk.Text(display_frame, height=15, bg="#2c3e50", fg="#ecf0f1",
    ...
    def setup_serial(self):
        ...
        try:
            # Note: The timeout is set to a low value to allow the thread to check the stop event
            self.ser = serial.Serial(self.serial_port, self.baud_rate, timeout=0.1)
            time.sleep(2)  # Give the connection time to establish
            self.append_to_display(f"Connected to {self.serial_port} at {self.baud_rate} baud.\n")
            ...
            messagebox.showerror("Serial Connection Error", f"Failed to connect to {self.serial_port}:\n{e}")
            self.ser = None
    ...
    def _send_command(self, command):
        """Helper function to send a command and display it in the monitor."""
        try:
            self.append_to_display(f"Sending: {command.strip()}")
            self.ser.write(command.encode('utf-8'))
        except Exception as e:
            self.append_to_display(f"Communication error: {e}")
            self.root.after_idle(self.handle_timeout) # Use after_idle to call on main thread
    ...
    def move_motor(self, motor_number):
        ...
        command = f"move motor {motor_number} abs {angle}\n"
